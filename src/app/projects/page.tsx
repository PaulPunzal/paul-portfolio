    ...
    <span
      className={`inline-block px-2.5 py-1 rounded-md font-mono text-[10px] tracking-[0.4px] border ${
        highlight
          ...
          : "bg-[rgb(var(--ink)/4%)] border-[rgb(var(--ink)/8%)] text-[rgb(var(--ink)/50%)]"
      }`}
    >
      {label}
    </span>
    ...
    <div className="w-full max-w-[1200px] mx-auto px-4 pb-16 pt-6 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col lg:flex-row gap-5 lg:gap-6 lg:items-start">
        ...
        <div className="lg:w-[340px] shrink-0 flex flex-col gap-3 lg:sticky lg:top-28">
                ...
                >
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <div className="flex items-center gap-2.5">
                      <span
                        className={`font-mono text-[10px] font-medium tabular-nums ${
                          isActive ? "text-accent" : "text-[rgb(var(--ink)/25%)]"
                        }`}
                      >
                        {padIndex(i)}
                      </span>
                      <span
                        className={`font-mono text-[9px] tracking-[1.5px] uppercase ${
                          isActive ? "text-accent/70" : "text-[rgb(var(--ink)/30%)]"
                        }`}
                      >
                        {project.label}
                      </span>
                    ...
                    <span
                      className={`w-1.5 h-1.5 rounded-full transition-all duration-300 ${
                        isActive ? "bg-accent scale-100" : "bg-[rgb(var(--ink)/10%)] scale-75 group-hover:scale-100 group-hover:bg-[rgb(var(--ink)/20%)]"
                      }`}
                    />
                  </div>
                  ...
                  <h3
                    className={`font-syne font-bold text-lg leading-tight transition-colors duration-200 ${
                      isActive ? "text-[var(--text-primary)]" : "text-[rgb(var(--ink)/60%)] group-hover:text-[rgb(var(--ink)/80%)]"
                    }`}
                  >
                    {project.title}
                  </h3>
                  ...
                  >
                    {project.previewTags.map((tag) => (
                      <span
                        key={tag}
                        className="font-mono text-[9px] text-[rgb(var(--ink)/40%)] bg-[rgb(var(--ink)/3%)] border border-[rgb(var(--ink)/7%)] rounded px-2 py-0.5"
                      >
                        {tag}
                      </span>
                    ))}
          ...
          {/* ── Footer note ─────────────────────────────────────────────── */}
          <p className="font-mono text-[9px] text-[rgb(var(--ink)/20%)] text-center pt-1 tracking-[1px] hidden lg:block">
        ...
        >
          ...
          <div className="border-b border-[rgb(var(--ink)/7%)] p-5 lg:p-7">
            <div className="flex items-start justify-between gap-4 mb-4">
              {/* Lucide icon mapping */}
              <div
                className="w-12 h-12 rounded-xl flex items-center justify-center shrink-0 border border-[rgb(var(--ink)/7%)]"
                style={{ background: active.iconBgStyle }}
              >
                {iconMap[active.iconName] || <Cpu className="w-6 h-6 text-[rgb(var(--ink)/50%)]" strokeWidth={1.5} />}
              </div>

              {/* Index badge */}
              <span className="font-mono text-[11px] text-[rgb(var(--ink)/20%)] tabular-nums mt-0.5">
            ...
            <h2 className="font-syne font-bold text-2xl md:text-3xl text-[var(--text-primary)] leading-tight mb-2.5">
              {active.fullTitle}
            </h2>
            <p className="font-inter text-sm text-[rgb(var(--ink)/55%)] leading-relaxed font-light mb-5">
              {active.fullDesc}
            </p>
                ...
                {active.githubUrls.map((repo, idx) => {
                  ...
                  return (
                    <Link 
                      ...
                      className="flex flex-1 sm:flex-none items-center justify-center sm:justify-start gap-2 px-3 sm:px-3.5 py-1.5 rounded-lg bg-[rgb(var(--ink)/3%)] hover:bg-[rgb(var(--ink)/8%)] border border-[rgb(var(--ink)/8%)] hover:border-[rgb(var(--ink)/20%)] transition-all group"
                    >
                      {isGithub ? (
                        /* GitHub Icon */
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4 sm:w-5 sm:h-5 text-[rgb(var(--ink)/30%)] group-hover:text-[var(--text-primary)] transition-colors shrink-0">
                      ...
                      <span className="font-mono text-[9px] sm:text-[10px] uppercase tracking-[1px] text-[rgb(var(--ink)/60%)] group-hover:text-[var(--text-primary)] pt-0.5 whitespace-nowrap overflow-hidden text-ellipsis">
                        {repo.label}
                      </span>
                    </Link>
                  );
          ...
          {/* ── Bullets ─────────────────────────────────────────────────── */}
          <div className="p-5 lg:p-7 border-b border-[rgb(var(--ink)/7%)]">
            <p className="font-mono text-[9px] tracking-[1.8px] uppercase text-[rgb(var(--ink)/30%)] mb-4">
              Key Contributions
            </p>
          ...
          {active.images && active.images.length > 0 && (
            <div className="p-5 lg:p-7 border-b border-[rgb(var(--ink)/7%)]">
              <div className="flex items-center justify-between mb-4">
                <p className="font-mono text-[9px] tracking-[1.8px] uppercase text-[rgb(var(--ink)/30%)]">
                  Project Gallery
                </p>
                <div className="flex items-center gap-1.5 font-mono text-[9px] tracking-[1px] uppercase text-[rgb(var(--ink)/40%)] animate-pulse">
                  <span>Scroll</span>
                  <span>→</span>
                </div>
              </div>
              ...
              <div className="relative">
                <div className="flex gap-3.5 overflow-x-auto pb-3 snap-x snap-mandatory scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
                  {active.images.map((img, idx) => (
                    <div 
                      key={idx} 
                      className="relative h-[280px] sm:h-[360px] shrink-0 snap-center rounded-xl overflow-hidden border border-[rgb(var(--ink)/8%)] bg-[var(--surface-0)] flex items-center justify-center p-1.5"
                    >
                ...
                <div className="absolute right-0 top-0 bottom-3 w-20 md:w-28 bg-gradient-to-l from-[var(--surface-1)] via-[var(--surface-1)]/80 to-transparent pointer-events-none flex items-center justify-end pr-2 md:pr-4">
                  <div className="w-8 h-8 md:w-9 md:h-9 rounded-full bg-[rgb(var(--ink)/5%)] backdrop-blur-md border border-[rgb(var(--ink)/10%)] flex items-center justify-center text-[rgb(var(--ink)/60%)] shadow-xl animate-pulse">
          ...
          <div className="p-5 lg:p-7">
            <p className="font-mono text-[9px] tracking-[1.8px] uppercase text-[rgb(var(--ink)/30%)] mb-3.5">
              Tech Stack
            </p>
          ...
          <div className="flex items-center justify-between px-5 lg:px-7 pb-5 lg:pb-7 pt-0">
            <button
              ...
              className="flex items-center gap-1.5 font-mono text-[10px] tracking-[1px] text-[rgb(var(--ink)/30%)] hover:text-[rgb(var(--ink)/70%)] disabled:opacity-20 disabled:cursor-not-allowed transition-colors"
            >
              <span>←</span>
              <span>PREV</span>
            </button>
                ...
                <button
                  ...
                  className={`rounded-full transition-all duration-300 ${
                    p.id === activeId
                      ...
                      : "w-1.5 h-1.5 bg-[rgb(var(--ink)/20%)] hover:bg-[rgb(var(--ink)/40%)]"
                  }`}
