...
export default function HomePage() {
  ...
  // 1. Anime.js Text & Icon Animations
  useEffect(() => {
    if (isLoading) return; // Do not run animations while splash screen is active
  ...
  // 2. Framer Motion variant for the staggering grid
  const gridVariants = {
    hidden: { opacity: 0 },
    visible: {
      opacity: 1,
  ...
  return (
    <>
      ...
      {/* ── ACTUAL PORTFOLIO PAGE (Only renders when splash is done) ── */}
      {!isLoading && (
        <motion.div 
          initial={{ opacity: 0 }} 
          animate={{ opacity: 1 }} 
                    ...
                    <h1 className="font-syne text-2xl sm:text-3xl lg:text-4xl font-bold tracking-tight mb-1.5 lg:mb-2 flex flex-wrap gap-[0.25em]">
                      {"Paul John Punzal".split(" ").map((word, i) => (
                        <span key={i} className="relative overflow-hidden inline-flex pb-1">
                          <span className="text-white/20 inline-block relative z-0">
                            {word}
                          </span>
                      ...
                      <span className="font-mono text-[9px] sm:text-[10px] tracking-[1.5px] sm:tracking-[2px] uppercase flex flex-wrap gap-[0.4em]">
                        {"Full-Stack & Mobile Dev".split(" ").map((word, i) => (
                          <span key={i} className="relative overflow-hidden inline-flex pb-0.5">
                            <span className="text-white/30 inline-block relative z-0">
                              {word}
                            </span>
