...
export default function Footer() {
  ...
  return (
    ...
    <footer className="relative w-full border-t border-[rgb(var(--ink)/5%)] py-6 mt-8 overflow-hidden bg-[var(--surface-1)]">
      ...
      <div className="relative z-10 max-w-[1200px] mx-auto px-4 flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="flex flex-col items-center md:items-start text-center md:text-left">
          <span className="font-syne font-bold text-[rgb(var(--ink)/90%)] text-lg tracking-tight">
            Paul Punzal
          </span>
          <span className="font-mono text-[10px] text-[rgb(var(--ink)/40%)] tracking-[1px] uppercase mt-1">
            © {new Date().getFullYear()} — All Rights Reserved.
          </span>
        </div>

        {/* Center: System Status / Easter Egg */}
        <div className="hidden md:flex items-center gap-2 font-mono text-[9px] text-[rgb(var(--ink)/30%)] uppercase tracking-[2px]">
        ...
        <div className="flex items-center gap-5">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="text-[rgb(var(--ink)/40%)] hover:text-accent transition-colors"
            aria-label="GitHub"
          >
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="w-7 h-7 text-[rgb(var(--ink)/20%)] self-end group-hover:text-accent transition-colors">
          ...
          {/* Subtle Divider */}
          <div className="w-[1px] h-4 bg-[rgb(var(--ink)/10%)] mx-1"></div>

          <button
            onClick={scrollToTop}
            className="group flex items-center gap-2 text-[rgb(var(--ink)/40%)] hover:text-accent transition-colors cursor-pointer"
          >
            <span className="font-mono text-[10px] uppercase tracking-wider hidden sm:block">Top</span>
            <div className="w-7 h-7 rounded-full border border-[rgb(var(--ink)/10%)] flex items-center justify-center group-hover:border-accent/40 transition-colors bg-[rgb(var(--ink)/2%)]">
