...
export default function FloatingResume() {
  ...
  return (
    ...
    >
      {/* ── Rotating SVG Text ── */}
      <div className="absolute inset-0 animate-[spin_10s_linear_infinite] text-[rgb(var(--ink)/40%)] group-hover:text-accent transition-colors duration-500 resume-rotator">
      ...
      <div className="w-12 h-12 sm:w-14 sm:h-14 bg-[var(--surface-1)]/80 backdrop-blur-xl border border-[rgb(var(--ink)/10%)] rounded-full flex items-center justify-center group-hover:border-accent/50 group-hover:bg-accent/10 transition-all duration-500 shadow-[0_0_30px_rgba(0,0,0,0.5)]">
        <Download className="w-5 h-5 sm:w-6 sm:h-6 text-[rgb(var(--ink)/70%)] group-hover:text-accent group-hover:translate-y-0.5 transition-all duration-300" />
      </div>
    </a>
  );
}