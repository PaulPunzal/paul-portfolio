...
export default function Navbar() {
  ...
  useEffect(() => {
    if (pathname !== "/") {
      setIsReady(true); // Animate instantly on other pages
      return;
    }

    // On homepage, wait for Phase 2 of the splash screen to fire!
    const handleReveal = () => setIsReady(true);
    window.addEventListener("revealNav", handleReveal); // CHANGED TO revealNav
    
    // Fallback just in case splash screen is bypassed
    const timer = setTimeout(() => setIsReady(true), 1500);

    return () => {
      window.removeEventListener("revealNav", handleReveal); // CHANGED TO revealNav
      clearTimeout(timer);
    };
  }, [pathname]);
  ...
  return (
    <>
      ...
      >
        <nav className="flex items-center justify-between px-3 py-2 bg-[#0a0a0a]/50 backdrop-blur-md border border-white/10 rounded-full shadow-2xl shadow-black/50">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link href="/contact" className="font-mono text-[11px] uppercase tracking-[1px] font-bold text-[#000] bg-accent px-5 py-2.5 rounded-full hover:bg-accent/90 hover:scale-105 transition-all duration-300">
              Contact Me
            </Link>
          </div>
        </nav>
      ...
      {/* ── MOBILE NAVBAR ── */}
      <motion.div 
        className="sm:hidden fixed bottom-6 left-1/2 z-50 w-[90%] max-w-[400px]"
        initial={{ x: "-50%", scaleX: 0, opacity: 0 }}
        animate={{ x: "-50%", scaleX: isReady ? 1 : 0, opacity: isReady ? 1 : 0 }}
        transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
        style={{ transformOrigin: "center" }}
      >
        <nav className="flex items-center justify-between px-2 py-2 bg-[#0c0c0c]/80 backdrop-blur-xl border border-white/10 rounded-full shadow-[0_8px_32px_rgba(0,0,0,0.8)]">
          {navLinks.map((link) => {
            ...
            return (
              <Link key={link.name} href={link.href} className={`flex-1 flex flex-col items-center justify-center gap-1 py-2 rounded-full transition-all duration-300 ${isActive ? "bg-white/10 text-white" : "text-white/40 hover:text-white/70"}`}>
          ...
          <Link href="/contact" className={`flex-1 flex flex-col items-center justify-center gap-1 py-2 rounded-full transition-all duration-300 ${pathname === "/contact" ? "bg-accent/20 text-accent" : "text-accent/80 hover:text-accent"}`}>
